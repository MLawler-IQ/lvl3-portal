...
| 1 · Question taxonomy + coverage-by-question | not started | |
...
|---|---|---|---|---|---|
| 1 | **Question taxonomy + coverage-by-question** | The result view and text report lead with seven question rows ("Measurement: 1 of 7 evaluated · Visibility: not measured"), never "N of 80". | EDIT `docs/rubric/rubric.json` (add `question` to all 80 rows); NEW `lib/audit/questions.ts` (pure partition; exhaustive + exclusive over active rows); EDIT `lib/orchestrator/report-text.ts`, `components/audit/AuditResultView.tsx`; NEW `tests/unit/audit-questions.test.ts` | A test asserts every active row lands in exactly one question and that the per-question totals sum to the active-row count. Tornado run re-read: same findings, new framing. | none |
| 2 | **Rubric re-cut** | Retired rows render nowhere; severity fixes live. Coverage numerators/denominators shift and the report says why. | EDIT `docs/rubric/rubric.json` (retire + edit per the list below); EDIT `lib/scoring/rubric.ts` consumers if any assume 80; RE-BASELINE both `scoring.snapshot.json` (ONPAGE-003 severity high→low moves its severity_weight 6→1); bump `SCORING_CONFIG.version` | The snapshot diff reviewed line by line. No registered check is retired (fact 1), so `CHECKS`, fixtures' `must_find`/`must_pass` lists, and detectors are untouched — a test run proves it. | none |
