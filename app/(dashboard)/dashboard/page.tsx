...
export default async function DashboardPage({
  ...
  type ClientRow = {
    ...
    snapshot_insights_draft: import("@/app/actions/analytics").SnapshotInsightsDraft | null;
  ...
  const selectedClient = selectedClientId
    ? await getClientById<ClientRow>(
        selectedClientId,
        "id, name, looker_embed_url, snapshot_insights, snapshot_insights_draft, analytics_summary_updated_at, client_type, targets, competitors, key_event_names"
      )
    : null;
  ...
  return (
    <Suspense key={`${selectedClient.id}-${period}-${compare}`} fallback={<DashboardLoading />}>
      <AnalyticsSection
        ...
        snapshotInsightsDraft={selectedClient.snapshot_insights_draft ?? null}
